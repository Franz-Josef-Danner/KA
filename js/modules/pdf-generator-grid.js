// -----------------------------
// PDF Generation Module - Grid-Based
// -----------------------------
...
export async function generatePDF(documentType, documentData, useSampleCompanyData = false) {
  ...
  try {
    if (layoutTemplate.grid) {
      renderPDFDocumentGrid(doc, documentType, documentData, companySettings, layoutTemplate);
    } else {
...
function renderPDFDocumentGrid(doc, documentType, documentData, companySettings, layoutTemplate) {
  ...
  layoutTemplate.grid.rows.forEach((row, rowIndex) => {
    ...
    row.cells.forEach((cell, cellIndex) => {
      ...
      currentX += colWidth * (cell.colSpan || 1);
    });
...
// Render individual element in grid system
function renderElementGrid(doc, elementType, x, y, width, height, documentType, documentData, companySettings) {
...
// Fallback: Render PDF document based on absolute positioning
...
// Render individual element (absolute positioning fallback)
function renderElement(doc, element, documentType, documentData, companySettings) {
  ...
  renderElementGrid(doc, element.type, x, y, width, height, documentType, documentData, companySettings);
}
