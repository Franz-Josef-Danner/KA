...
function renderItemsTable(doc, x, y, width, height, documentData) {
  ...
  items.forEach((item, index) => {
    ...
    // Only apply minimum height if row is truly empty (no description text or only whitespace)
    const rowHeight = beschreibungText.trim() ? calculatedHeight : Math.max(minRowHeight, calculatedHeight);
