...
// Function to generate invoice ID based on Firmen_ID
function generateInvoiceId(firmenId) {
  if (!firmenId) {
    return "RE-" + Date.now();
  }
...
function renderInvoiceItemsTable() {
  ...
  currentInvoiceItems.forEach((item, idx) => {
    ...
    datumInput.addEventListener("change", (e) => {
      ...
      currentInvoiceItems[itemIndex].Datum = e.target.value;
    });
    ...
    mengeInput.addEventListener("input", (e) => {
      ...
      if (row) {
        ...
        if (gesamtpreisTd) {
          gesamtpreisTd.textContent = (currentInvoiceItems[itemIndex].Gesamtpreis || "0.00") + " €";
        }
      }
    });
...
function onCompanySelectionChange(event) {
  ...
  if (selectedFirmenId) {
    ...
    if (currentEditingRowIndex === null) {
      const rechnungsIdInput = document.getElementById("edit_Rechnungs_ID");
      if (rechnungsIdInput) {
        rechnungsIdInput.value = generateInvoiceId(selectedFirmenId);
      }
    }
  } else {
    hideCompanyInfo();
...
function getFormData() {
  ...
  for (const col of COLUMNS) {
    ...
    // Handle Firmen_ID – the select value is already the ID
    if (col === "Firmen_ID") {
      formData[col] = selectedCompany ? selectedCompany.Firmen_ID : "";
      continue;
    }
