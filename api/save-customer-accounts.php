<?php
/**
 * API Endpoint: Save Customer Accounts (Kundenkonten)
 * 
 * Receives JSON data from the frontend and saves it to a JSON file
 * Customer accounts are used for customer login to their Kundenbereich
 */
...
$dataFile = $dataDir . '/customer-accounts.json';
...
if (file_exists($dataFile)) {
    $backupFile = $dataDir . '/customer-accounts.backup.json';
    if (!copy($dataFile, $backupFile)) {
        // Log backup failure but continue with save
        error_log("Warning: Failed to create backup of customer-accounts.json");
    }
}
