<?php
/**
 * API Endpoint: Save Email Configuration
 * 
...
if (!isset($data['enabled'])) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'Enabled field is required']);
    exit;
}

// Path to the data file
$dataDir = __DIR__ . '/../data';
$dataFile = $dataDir . '/email-config.json';
...
if (file_exists($dataFile)) {
    $backupFile = $dataDir . '/email-config.backup.json';
    if (!copy($dataFile, $backupFile)) {
        // Log backup failure but continue with save
        error_log("Warning: Failed to create backup of email-config.json");
    }
}
