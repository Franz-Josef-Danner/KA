<?php
/**
 * API Endpoint: Save Artikellisten (Article Lists)
 * 
 * Receives JSON data from the frontend and saves it to a JSON file
 * Article lists are keyed by Firmen_ID
 */
...
// Ensure data is an object (article lists are stored as objects with Firmen_ID keys)
if (!is_array($data) || !is_object(json_decode(json_encode($data)))) {
    http_response_code(400);
...
$dataFile = $dataDir . '/artikellisten.json';
...
if (file_exists($dataFile)) {
    $backupFile = $dataDir . '/artikellisten.backup.json';
    if (!copy($dataFile, $backupFile)) {
        // Log backup failure but continue with save
        error_log("Warning: Failed to create backup of artikellisten.json");
    }
}
